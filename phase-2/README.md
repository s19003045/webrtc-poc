...
| 階段 | 痛點 | 決策 / 解法 |
|------|------|------------|
| 要不要趁機換 React | 多人功能想做，而技術棧最終本來就要換 React。 | 刻意先不換，留在原生 JS——這階段只專注「同時管理多條 PeerConnection」這個 WebRTC 本身的難點。 |
| 信令定址 | Phase 1 的「轉給另一個人」在 N>2 時無法指定對象。 | 每條連線給唯一 peer ID，訊息帶 `to` 做點對點轉發。 |
| 多人 glare | 多人同時進房，誰對誰發 offer 容易衝突或重複協商。 | 規定「後加入者對每個既有成員發 offer」，每對之間只有單向。 |
| ICE 佇列 | Phase 1 的單一 candidate 佇列在多 peer 下會混在一起。 | 改成每個 peer 各一個 pending 佇列。 |
| 螢幕分享 | 用重建連線來換畫面會讓所有 peer 重新協商、失敗面擴大。 | 用 `sender.replaceTrack()` 直接換 video 軌，免重新協商。 |
| TURN | 想驗證跨網路，但架 coturn 在本機無法有意義地驗證。 | 只在 `rtcConfig` 留設定槽 + 文件說明，不在本階段實際架設。 |
